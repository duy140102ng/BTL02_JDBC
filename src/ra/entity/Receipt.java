...
public class Receipt {
    ...
    public Receipt(long billId, String billCode, boolean billType, String empIdCreated, Date created, String empIdAuth, Date authDate, int quantity, float price, long billDetailId, String productId, int billStatus) {
        this.billId = billId;
    ...
    private static boolean checkDuplicateIdAuth(String receiptId) {
        Connection conn = ConnectionDB.openConnection();
        CallableStatement callSt = null;
        try {
            callSt = conn.prepareCall("{call createBillId(?, ?)}");
    ...
    private static boolean checkDuplicateProductId(String receiptId) {
        Connection conn = ConnectionDB.openConnection();
        CallableStatement callSt = null;
        try {
    ...
    public void inputDataBrowse(Scanner scanner) {
    ...
    public void displayDataBrowse() {
        System.out.printf("Mã phiếu: %d - Mã code: %s - Loại phiếu: %s\n", this.billId, this.billCode, (this.billType == true ? "Phiếu nhập" : "Phiếu xuất"));
        System.out.printf("Mã nhân viên duyệt: %s - Ngày duyệt: %s - Số lượng nhập: %d - Gía nhập: %f\n", this.empIdAuth, this.authDate, this.quantity, this.price);
        System.out.printf("Mã sản phẩm: %s - Trạng thái: %s\n",this.productId, (this.billStatus == 0 ? "Tạo" : (this.billStatus == 1 ? "Hủy" : "Duyệt")));
    }

    public void displayDataBillDetail() {
        System.out.printf("Mã phiếu chi tiết: %d - Mã phiếu nhập: %d - Mã sản phẩm: %s\n", this.billDetailId, this.billId, this.productId);
        System.out.printf("Số lượng nhập: %d - Gía nhập: %f\n", this.quantity, this.price);
    }
}
